...
const Aluno = ({ navigation }) => {
  ...
  const fetchAlunos = async () => {
    setLoading(true);
    try {
      const response = await fetch('https://bancoapialuno.azurewebsites.net/alunos');
      if (!response.ok) {
  ...
  const handleAddAluno = async () => {
    ...
    try {
      await fetch('https://bancoapialuno.azurewebsites.net/alunos', {
        method: 'POST',
  ...
  const handleUpdateAluno = async () => {
    const data = { nome, email, senha, turma };
    try {
      await fetch(`https://bancoapialuno.azurewebsites.net/alunos/${selectedId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
  ...
  const handleSelectAluno = async (id) => {
    try {
      const response = await fetch(`https://bancoapialuno.azurewebsites.net/alunos/${id}`);
      if (!response.ok) {
